...
export class Card {
     ...
     */
    public async renderSync(): Promise<PIXI.Container> {
        if (this.container) {
    ...
    private async renderImage(): Promise<void> {
        ...
        const backgroundImage = await newSprite(this.imageURL, this.container);
    ...
    private async renderBackground(): Promise<void> {
        ...
        const backgroundSprite = await newSprite(backgroundType.replace(" ", "-").toLowerCase(), this.container);
        // let bounds = backgroundSprite.getBounds();
        // bounds = backgroundSprite.getLocalBounds();
        // backgroundSprite.scale.set(1000, 1000);
        // let x = 714;
        // let y = 26;
        // // backgroundSprite.pivot.x = bounds.width;
    ...
    private async renderLogo(): Promise<void> {
        ...
        const logoSprite = await newSprite(this.logoURL, this.container);
    ...
    private async renderCost(): Promise<void> {
        ...
        await newSprite("background-cost", this.container);
     ...
     */
    private async renderVP(): Promise<void> {
        if (this.oversized) {
            return;
        }
        ...
        if (this.victoryPoints === "*") {
            await newSprite("vp-variable", this.container);
        }
        else { // it's a number
            const scalar = 2;
