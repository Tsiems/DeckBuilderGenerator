...
export class DeckBuilder extends EventEmitter {
    ...
    private parse(csv: string): Promise<Card[]> {
        return new Promise<Card[]>((resolve, reject) => {
            ...
            csvParse.parse(csv, csvOptions, (err: Error | undefined, data: any[]) => {
                if (err) {
    ...
    private renderAllCards(
        ...
        this.renderCards(normalCards, oversizedCards).then((app: PIXI.Application | null) => {
            ...
            app.render();
            app.view.toBlob!((blob: Blob | null) => {
                ...
                // for (const textureKey in PIXI.Loader.shared.resources) {
                //     if (!initialTextures.hasOwnProperty(textureKey)) {
                //         PIXI.Loader.shared.resources[textureKey].texture.destroy(true);
                //         delete PIXI.Loader.shared.resources[textureKey];
    ...
    private async renderCards(normalCards: Card[], oversizedCards: Card[]): Promise<PIXI.Application | null> {
        ...
        const renders = await Promise.all(currentCards.map((card) => card.renderSync()));
        ...
        const app = new PIXI.Application({ width: resizedWidth * maxWidth, height: resizedHeight * maxHeight, antialias: true, backgroundAlpha: 0 });
